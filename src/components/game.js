...
import initialiseChessBoard from "./../helpers/initialboard";
  ...
  handleClick(i) {
    ...
    if (this.state.sourceSelection === -1) {
      ...
      if (squares[i] && squares[i].player === this.state.player) {
        this.setState({
          status:
            "Wrong selection, choose a valid source and destination again.",
          sourceSelection: -1,
        });
      } else {
        const squares = this.state.squares.slice();
        ...
        if (isMovePossible && isMovePossible) {
          if (squares[i] !== null) {
      ...
      <div>
        ...
        <div className="icons-attribution">
          <div>
            ...
            <small>
              {" "}
              Chess Icons and Favicon are extracted from user, Cburnet [
